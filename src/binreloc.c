...
_br_find_exe (BrInitError *error)
{
	...
	*error = BR_INIT_ERROR_DISABLED;
    ...
    path[size] = '\0';
#elif defined(BR_FREEBSD)
    /* Check if procfs is currently mounted. */
    strncpy (path2, "/proc/curproc/file", buf_size - 1);
    size = readlink (path2, path, buf_size - 1);
    if (size == -1) {
	/* Use FreeBSD sysctl() to determine the process image path.
         * This method is specifically known not to work on
         * OpenBSD. */
	size_t len = buf_size;
	int name[4];
	name[0] = CTL_KERN;
	...
	if (sysctl (name, 4, path, &len, NULL, 0) == -1 ||
	    ...
	    free (path);
	    if (error)
		*error = BR_INIT_ERROR_READ_MAPS; /* ??? Not true. */
	    return NULL;
	}
    }
    else
	...
	/* Follow the symlink. We want to get the final target. */
	strcpy (path2, path);
	size = readlink (path2, path, buf_size - 1);
	if (size == -1) {
	    /* Error. */
    ...
    /* Not doing this because of note below. */
    free (path);
    if (error)
	*error = BR_INIT_ERROR_READ_MAPS;
    return NULL;
    ...
    /* The first entry should be the executable name. Note: On Ubuntu
     * 10.04 LTS with Linux 2.6.32, this does not work because the
     * first entry is not actually the executable name. */
 ...
 *	 will be returned. If default_prefix is NULL, then NULL will
 *	 be returned.
 */
char *
 ...
 * @return A string containing the bin folder's path, which must be
 *	 freed when no longer necessary. If BinReloc is not
 *	 initialized, or if the initialization function failed, then a
 *	 copy of default_bin_dir will be returned. If default_bin_dir
 ...
 *	 freed when no longer necessary. If BinReloc is not
 *	 initialized, or if the initialization function failed, then a
 *	 copy of default_sbin_dir will be returned. If default_bin_dir
 *	 is NULL, then NULL will be returned.
 */
char *
 ...
 *	 freed when no longer necessary. If BinReloc is not
 *	 initialized, or if the initialization function failed, then a
 *	 copy of default_data_dir will be returned. If
 *	 default_data_dir is NULL, then NULL will be returned.
 */
char *
 ...
 * @return A string containing the localization folder's path, which
 *	 must be freed when no longer necessary. If BinReloc is not
 *	 initialized, or if the initialization function failed, then a
 *	 copy of default_locale_dir will be returned.  If
 *	 default_locale_dir is NULL, then NULL will be returned.
 */
char *
 ...
 * @return A string containing the library folder's path, which must
 *	 be freed when no longer necessary. If BinReloc is not
 *	 initialized, or if the initialization function failed, then a
 *	 copy of default_lib_dir will be returned.  If default_lib_dir
 *	 is NULL, then NULL will be returned.
 */
char *
 ...
 * @return A string containing the libexec folder's path, which must
 *	 be freed when no longer necessary. If BinReloc is not
 *	 initialized, or if the initialization function failed, then a
 *	 copy of default_libexec_dir will be returned.  If
 *	 default_libexec_dir is NULL, then NULL will be returned.
 */
char *
 ...
 * @return A string containing the etc folder's path, which must be
 *	 freed when no longer necessary. If BinReloc is not
 *	 initialized, or if the initialization function failed, then a
 *	 copy of default_etc_dir will be returned.  If default_etc_dir
 *	 is NULL, then NULL will be returned.
 */
char *
 ...
 * @returns A newly-allocated string. This string should be freed when
 * no longer needed.
 */
char *
 ...
 *
 * @param path  A path.
 * @returns A directory name. This string should be freed when no
 * longer needed.
 */
char *
